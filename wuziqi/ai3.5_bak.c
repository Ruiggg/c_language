...
//参数：记录连珠信息的棋盘（ai_Bo...:player方的棋盘，man_Bo...对方的棋盘），现在的棋盘，执子的颜色，如果这个子被禁或者被占，那么把分数修改为-1
...
{	
	...
	//深度是0就不搜索	
	if(deep==0){
		cal_situation(t,aRecordBoard2,ai_BoardScore);//计算ai的状态信息
		cal_situation(anti,aRecordBoard2,man_BoardScore);//计算玩家的状态信息
		cal_score(t,first,ai_BoardScore);//计算AI每个点的得分
		cal_score(anti,first,man_BoardScore);//计算玩家每个点的得分
		change_score(ai_BoardScore,man_BoardScore,aRecordBoard2,t);//判断禁手，改变二者的得分
		loc1=find_max(ai_BoardScore,man_BoardScore);//找到最大值点
		return loc1;//返回最大值点
	}else{
		struct loc candidate[NUM];//用来记录候选的AI的棋子
		select_point(t,first,NUM,candidate,aRecordBoard2,ai_BoardScore,man_BoardScore);//找出AI的NUM个候选点，不够的话剩余位置填（-1，-1）
		int score_of_candt[NUM];//记录每个候选点的分数
		int n,step=0;
		for(n=0;n<NUM;n++)
			if(candidate[n].i>=0 && candidate[n].j>=0)//如果被填上了
...
//选点函数，输入t棋子颜色，first谁先谁后，num暂时没用，候选子数组，棋盘，两个记录每点状态的数组
...
int cal_Score_of_Bd(struct SingleScore ai_BoardScore[15][15],struct SingleScore man_BoardScore[15][15])
{
	int live[6];
	int half[6];
	int dead[6];
